...
- (void)runGameWithGameRoot:(NSString *)gameRoot
    ...
    @autoreleasepool {
        ...
        if (useExperimentalQoS) {
            SsbmPadLog(@"runtime performance profile=%@ cpuVideoSplit=0 shaderCompilerThreads=3 emulatedCPUClock=%.2f gameThreadQoS=userInitiated qosResult=%d source=%@",
                      performanceProfile, emulatedCPUClock, qosResult, performanceSource);
        } else {
            SsbmPadLog(@"runtime performance profile=stable cpuVideoSplit=0 shaderCompilerThreads=3 emulatedCPUClock=1.00 gameThreadQoS=inherited source=default");
        }
        @synchronized (self) {
