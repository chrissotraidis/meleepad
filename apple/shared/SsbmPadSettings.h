...
typedef NS_ENUM(NSInteger, SsbmPadAspectRatioMode) {
    SsbmPadAspectRatioOriginal = 0,
    SsbmPadAspectRatioWidescreen = 1,
    SsbmPadAspectRatioFillScreen = 2,
};

/* Persisted SsbmPad settings shared by macOS, iOS, and iPadOS. Stored in
 * NSUserDefaults so each platform keeps the same user-facing options.
 */
@interface SsbmPadSettings : NSObject
...
@property(nonatomic, assign) SsbmPadAspectRatioMode aspectRatioMode;
