...
- (BOOL)hasNativeModuleForRevision:(NSInteger)revision slippi:(BOOL)slippi;
...
#if !TARGET_OS_SIMULATOR
    // Legacy provisioning files may contain Simulator-only host paths.
...
- (void)settingsChanged:(NSNotification *)notification {
    // NSUserDefaults notifications can originate on system background threads.
    if (![NSThread isMainThread]) {
        dispatch_async(dispatch_get_main_queue(), ^{ [self settingsChanged:notification]; });
        return;
    }
    ...
    [_slippiHost setRenderScale:settings.renderScale];
    [_slippiHost setAspectRatioMode:settings.aspectRatioMode];
...
- (void)playFromHome {
    ...
    if ([self availableDiscImagePathForRevision:revision].length == 0) {
        [self presentGameDataImport];
        return;
    }
    if (![self hasNativeModuleForRevision:revision slippi:NO]) {
        [self presentOriginalModuleSetup];
        return;
    }
    _slippiRequested = NO;
    _playRequested = YES;
...
- (void)startSlippiFromCurrentGame {
    if (_slippiHost.isRunning) {
        UIAlertController *status = [UIAlertController
            alertControllerWithTitle:@"Slippi Multiplayer"
            message:[NSString stringWithFormat:@"Status when opened:\n%@\n\nRanked, Unranked, Direct, Teams, and Party are enabled for testing. Account eligibility and mode availability are determined by Slippi.",
                     [_slippiHost matchmakingStatusSummary]]
            preferredStyle:UIAlertControllerStyleAlert];
        [status addAction:[UIAlertAction actionWithTitle:@"Return to Game"
...
- (BOOL)hasNativeModuleForRevision:(NSInteger)revision slippi:(BOOL)slippi {
    // The resolver reads the selected revision from Settings. Check that the
    // Home revision matches it before claiming either module is available.
    if (revision < 0 || revision != [MeleePadSettings sharedSettings].gameRevision ||
        (slippi && revision != 2))
        return NO;
    ...
    NSString *modulePath = [self modulePathFromConfiguration:config forSlippi:slippi];
...
- (BOOL)checkSlippiInstallForRevision:(NSInteger)revision {
    BOOL moduleReady = [self hasNativeModuleForRevision:revision slippi:YES];
...
- (void)showHomeForRevision:(NSInteger)revision {
    ...
    BOOL discReady = gameFilesReady &&
        [self availableDiscImagePathForRevision:revision].length > 0;
    BOOL gameDataReady = gameFilesReady && discReady;
    BOOL originalModuleReady = [self hasNativeModuleForRevision:revision slippi:NO];
    BOOL originalReady = gameDataReady && originalModuleReady;
    BOOL slippiModuleReady = [self hasNativeModuleForRevision:revision slippi:YES];
    ...
    eyebrow.text = originalReady || (gameDataReady && slippiModuleReady)
        ? @"CHOOSE YOUR EXPERIENCE" : @"BUILD YOUR SETUP";
    ...
    _homeStatusLabel.text = gameDataReady
        ? [NSString stringWithFormat:@"%@  ·  Game data ready", MeleePadRevisionLabel(revision)]
    ...
    _homeStatusLabel.textColor = gameDataReady ? [UIColor colorWithRed:0.42 green:0.89 blue:0.75 alpha:1] : muted;
    ...
    [choices addArrangedSubview:(makeChoiceCard(
        @"Original Melee",
        originalReady ? [NSString stringWithFormat:@"%@ ready · standalone play", MeleePadRevisionLabel(revision)]
              : gameDataReady ? @"Matching native game module missing · build a complete app."
        ...
        originalReady ? @"PLAY MELEE" : gameDataReady ? @"BUILD TO PLAY" : @"IMPORT GAME DATA",
        [UIImage systemImageNamed:originalReady ? @"play.fill" :
            gameDataReady ? @"hammer.fill" : @"square.and.arrow.down"],
        amber,
        originalReady ? @selector(playFromHome) :
            gameDataReady ? @selector(presentOriginalModuleSetup) : @selector(presentGameDataImport),
        originalReady ? @"home.play" : gameDataReady ? @"home.build" : @"home.import"))];
...
- (void)startGameIfProvisioned {
    ...
    if (_slippiRequested) {
        ...
        [_slippiHost startWithGameRoot:gameRoot
            ...
            MeleePadSettings *settings = [MeleePadSettings sharedSettings];
            [strongSelf->_slippiHost setRenderScale:settings.renderScale];
            [strongSelf->_slippiHost setAspectRatioMode:settings.aspectRatioMode];
            strongSelf->_bootStatusLabel.accessibilityLabel = @"Slippi runtime active";
            // Runtime readiness is useful in the diagnostic log, but this
            // startup label must not remain over the actual game surface.
            strongSelf->_bootStatusLabel.hidden = YES;
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (_pendingBootError != nil) {
        NSString *message = _pendingBootError;
        _pendingBootError = nil;
...
- (void)presentBootError:(NSString *)message {
    ...
    _bootStatusLabel.accessibilityLabel = _bootStatusLabel.text;
    [_bootActivityIndicator stopAnimating];
    // A startup prerequisite can fail during viewDidLoad. UIKit drops alerts
    // presented before this controller enters the window hierarchy.
    if (self.view.window == nil) {
        _pendingBootError = [message copy];
        return;
    }
...
- (void)gameOverlayRequestsOnlinePlay:(MeleePadGameOverlay *)overlay {
    if (_slippiRequested || _slippiHost.isRunning) {
        [self startSlippiFromCurrentGame];
        return;
    }
    (void)overlay;
