...
- Three-dot menu: render scale 1x-4x (live via `Config::GFX_EFB_SCALE`), aspect (original 4:3 default; 16:9 and Fill as experimental), FPS counter, controller mapping, touch layout edit/reset, Game Data & Saves (import/reimport/remove via Files), a single privacy-bounded Report a Problem flow, and Offline Cheats with the unlock-all toggle. Offline cheats must be removed before netplay boot, and performance experiments must not appear as product settings.
