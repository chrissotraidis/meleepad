...
| 8 | Save/memory card | Pass | Live `CODM` (macOS) and `CODX` (iPad Simulator) names were visibly recovered after clean process relaunches. MeleePad settings were also read back after relaunch on both targets. See the row artifact. |
...
| 10 | Menu system parity | Pass | G7 live-proves resolution, aspect, FPS, touch settings/reset, mapping, and diagnostics. Row 13 proves document-picker/folder import and reimport. A live destructive run subsequently stopped the runtime, removed only GameData, preserved the separate memory-card path, showed legal setup, reimported the exact pinned hash from the MeleePad folder, extracted 1,209 files, left zero staging directories, and visibly rebooted. Experimental Performance was toggled on/off with its restart explanation and restored off. |
