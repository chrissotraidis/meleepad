...
#if TARGET_OS_SIMULATOR
    if (error != nullptr)
        *error = @"Native Slippi is available in the iPhoneOS build, not the simulator build.";
    return NO;
#else
...
#if TARGET_OS_SIMULATOR
    if (onError != nil)
        onError(@"Native Slippi is available in the iPhoneOS build, not the simulator build.");
    return;
#else
