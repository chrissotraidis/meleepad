...
| MX11 | Development pass | Topic-based global help, contextual mode help, and collapsible README answers now explain why Public Games fails closed without a production HTTPS service. The unavailable state replaces disabled room controls with an explicit security boundary and a working Private Room action. Spoken VoiceOver remains. |
