...
@interface MeleePadStickView : UIView
...
@implementation MeleePadStickView {
    UIView *_thumb;
...
- (void)pulseAccessibilityX:(float)x y:(float)y value:(NSString *)value {
    ...
    __weak MeleePadStickView *weakSelf = self;
...
@property(nonatomic, copy) void (^accessibilityPressHandler)(MeleePadGameButton *button);
@end

@implementation MeleePadGameButton
...
static CGFloat const MeleePadTriggerDetentEnter = 0.75;
static CGFloat const MeleePadTriggerDetentExit = 0.70;
static CGFloat const MeleePadTriggerAnalogMinimum = 0.25;
static CGFloat const MeleePadTriggerAnalogMaximum = 0.89;

static CGRect MeleePadFrameAtNormalizedCenter(CGRect safe, CGFloat x, CGFloat y,
...
static NSString *const MeleePadExperimentalDPadOriginKey = @"MeleePadExperimentalDPadOrigin";
static NSString *const MeleePadExperimentalDPadScaleKey = @"MeleePadExperimentalDPadScale";

static BOOL MeleePadUsesPhoneLayoutDefaults(UIView *view) {
    return view.traitCollection.userInterfaceIdiom == UIUserInterfaceIdiomPhone;
}

static CGFloat MeleePadDefaultSizeScaleForControl(UIView *view, NSString *identifier) {
    return MeleePadUsesPhoneLayoutDefaults(view) && [identifier isEqualToString:@"B"]
        ? 1.158457040786743
        : 1.0;
}

@interface MeleePadTriggerButton : MeleePadGameButton
@property(nonatomic, copy) void (^pressureChanged)(uint8_t pressure, BOOL fullPress);
- (void)resetState;
@end

@implementation MeleePadTriggerButton {
...
- (void)updateDetentPath {
    ...
    CGFloat x = width * MeleePadTriggerDetentEnter;
...
- (void)updateFromTouch:(UITouch *)touch {
    ...
    _fullPress = wasFull ? position >= MeleePadTriggerDetentExit
                         : position >= MeleePadTriggerDetentEnter;
    CGFloat analogRange = MeleePadTriggerAnalogMaximum - MeleePadTriggerAnalogMinimum;
    _pressure = _fullPress ? 1.0 :
        MIN(MeleePadTriggerAnalogMaximum,
            MeleePadTriggerAnalogMinimum +
                (position / MeleePadTriggerDetentEnter) * analogRange);
...
@interface MeleePadDPadEditorGroup : UIView
@end

@implementation MeleePadDPadEditorGroup
@end

@interface MeleePadPassThroughView : UIView
@end

@implementation MeleePadPassThroughView
...
@interface MeleePadGameOverlay () <UIGestureRecognizerDelegate>
@end

@implementation MeleePadGameOverlay {
    UIButton *_menuButton;          // the three-dot menu
    MeleePadStickView *_moveStick;
    MeleePadStickView *_cStick;
    MeleePadTriggerButton *_rTriggerButton;
    MeleePadDPadEditorGroup *_experimentalDPadGroup;
    NSMutableArray<MeleePadGameButton *> *_buttons;
    ...
    MeleePadInputState _touchState;
...
- (UIMenu *)buildMenu {
    ...
    UIMenu *aspectMenu = [UIMenu menuWithTitle:@"Aspect Ratio" children:@[
        [self aspectRatioAction:@"Original 4:3" mode:MeleePadAspectRatioOriginal],
    ...
    UIMenu *dataMenu = [UIMenu menuWithTitle:@"Game Data & Saves"
        ...
        [UIAction actionWithTitle:@"Import from MeleePad Folder"
...
- (void)openGitHubReportWithID:(NSString *)reportID
    ...
    NSString *platform = self.traitCollection.userInterfaceIdiom == UIUserInterfaceIdiomPad ?
        @"iPad" : @"iPhone";
    NSString *problem = answers[@"problem"].length > 0 ? answers[@"problem"] : @"MeleePad problem";
    if (problem.length > 100)
        problem = [problem substringToIndex:100];
    NSURLComponents *components = [NSURLComponents
        componentsWithString:@"https://github.com/chrissotraidis/meleepad/issues/new"];
    ...
    [UIApplication.sharedApplication openURL:url options:@{}
        ...
        if (!success)
            MeleePadLog(@"diagnostic github open failed id=%@", reportID);
    }];
}

- (UIAction *)aspectRatioAction:(NSString *)title mode:(MeleePadAspectRatioMode)mode {
    __weak MeleePadGameOverlay *weakSelf = self;
    UIAction *aspectAction = [UIAction actionWithTitle:title
                                            ...
                                            identifier:nil
                                               handler:^(__kindof UIAction *action) {
        (void)action;
        [MeleePadSettings sharedSettings].aspectRatioMode = mode;
        [[MeleePadSettings sharedSettings] synchronize];
        [[[UISelectionFeedbackGenerator alloc] init] selectionChanged];
        [weakSelf refreshMenuButton];
    }];
    aspectAction.state = [MeleePadSettings sharedSettings].aspectRatioMode == mode ?
...
- (UIAction *)renderAction:(NSString *)title scale:(NSInteger)scale {
    ...
    UIAction *renderAction = [UIAction actionWithTitle:title
                                            ...
                                            identifier:nil
                                               handler:^(__kindof UIAction *action) {
        (void)action;
        [MeleePadSettings sharedSettings].renderScale = scale;
        [[MeleePadSettings sharedSettings] synchronize];
        [[[UISelectionFeedbackGenerator alloc] init] selectionChanged];
        [weakSelf refreshMenuButton];
    }];
    renderAction.state = [MeleePadSettings sharedSettings].renderScale == scale ?
        UIMenuElementStateOn : UIMenuElementStateOff;
    return renderAction;
}
...
- (void)buildTouchControls {
    ...
    [self addButton:@"A" mask:MeleePadButtonA];
    [self addButton:@"B" mask:MeleePadButtonB];
    [self addButton:@"X" mask:MeleePadButtonX];
    [self addButton:@"Y" mask:MeleePadButtonY];
    [self addButton:@"Z" mask:MeleePadButtonZ];
    [self addButton:@"START" mask:MeleePadButtonStart];
    [self addButton:@"L" mask:MeleePadButtonL];
    [self addButton:@"R" mask:MeleePadButtonR];
    // D-pad
    [self addButton:@"▲" mask:MeleePadButtonDpadUp];
    [self addButton:@"▼" mask:MeleePadButtonDpadDown];
    [self addButton:@"◀" mask:MeleePadButtonDpadLeft];
    [self addButton:@"▶" mask:MeleePadButtonDpadRight];

    _experimentalDPadGroup = [MeleePadDPadEditorGroup new];
...
- (MeleePadStickView *)makeStick {
    ...
    stick.valueChanged = ^(float x, float y) {
        MeleePadStickView *strongStick = weakStick;
        if (strongStick != nil)
...
- (void)addButton:(NSString *)label mask:(uint16_t)mask {
    ...
    __weak MeleePadGameOverlay *weakSelf = self;
    button.accessibilityPressHandler = ^(MeleePadGameButton *pressedButton) {
        MeleePadGameOverlay *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
...
- (void)stickChanged:(MeleePadStickView *)stick x:(float)x y:(float)y {
    if (_editingLayout)
        return;
    ...
    [[MeleePadInputMixer sharedMixer] setInputState:_touchState fromTouch:YES];
}

- (void)buttonDown:(MeleePadGameButton *)button {
    if (_editingLayout)
        return;
    _touchState.buttons |= button.inputMask;
    if (button.inputMask == MeleePadButtonL)
        _touchState.triggerL = 255;
    if (button.inputMask == MeleePadButtonR)
...
- (void)buttonUp:(MeleePadGameButton *)button {
    if (_editingLayout)
        return;
    _touchState.buttons &= ~button.inputMask;
    if (button.inputMask == MeleePadButtonL)
...
- (void)rPressureChanged:(uint8_t)pressure fullPress:(BOOL)fullPress {
    ...
    if (fullPress)
        _touchState.buttons |= MeleePadButtonR;
    else
        _touchState.buttons &= ~MeleePadButtonR;
    _touchState.connected = 1;
    [[MeleePadInputMixer sharedMixer] setInputState:_touchState fromTouch:YES];
...
- (void)clearTouchInput {
    for (MeleePadGameButton *button in _buttons)
        button.transform = CGAffineTransformIdentity;
    [_moveStick reset];
    [_cStick reset];
...
- (void)layoutSubviews {
    ...
    BOOL phone = MeleePadUsesPhoneLayoutDefaults(self);
    CGFloat baseScale = pad ? 1.0
                            : std::min<CGFloat>(1.0, std::min(safe.size.width / 800.0,
                                                              safe.size.height / 380.0));
    CGFloat controlScale = [MeleePadSettings sharedSettings].controlSizeScale;
    ...
    CGRect moveDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.1234722222, 0.7803490991, stick, stick) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.1310395315, 0.7905894519, stick, stick) :
    ...
    CGRect cameraDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.9233055556, 0.8130067568, camera, camera) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.9062957540, 0.8583247156, camera, camera) :
    ...
    MeleePadGameButton *a = [self buttonWithMask:MeleePadButtonA];
    MeleePadGameButton *b = [self buttonWithMask:MeleePadButtonB];
    MeleePadGameButton *x = [self buttonWithMask:MeleePadButtonX];
    MeleePadGameButton *y = [self buttonWithMask:MeleePadButtonY];
    ...
    CGRect aDefault = pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8916544656, 0.7409513961, large, large) :
    ...
    CGRect bDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8398611111, 0.6898648649, medium, medium) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8360175695, 0.8092037229, medium, medium) :
    ...
    CGRect xDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.9034166667, 0.4258445946, small, small) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.9593704246, 0.7156153051, small, small) :
    ...
    CGRect yDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8452500000, 0.5268581081, small, small) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.9542459736, 0.7869700103, small, small) :
    ...
    CGRect lDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.0905833333, 0.2539977477,
                                      shoulderWidth, small) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.1281112738, 0.6633919338,
                                      shoulderWidth, small) :
        CGRectMake(CGRectGetMinX(safe) + margin, shoulderY, shoulderWidth, small);
    [self placeControl:[self buttonWithMask:MeleePadButtonL]
          defaultFrame:lDefault
            identifier:@"L"];
    MeleePadGameButton *rightShoulder = [self buttonWithMask:MeleePadButtonR];
    CGRect rDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8687500000, 0.2729166667,
                                      shoulderWidth, small) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8960468521, 0.6478800414,
                                      shoulderWidth, small) :
    ...
    CGRect zDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.9712500000, 0.4350788288, small, small) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8275988287, 0.7213029990, small, small) :
        CGRectMake(CGRectGetMaxX(safe) - margin - shoulderWidth - small - 12.0 * scale,
                   shoulderY, small, small);
    [self placeControl:[self buttonWithMask:MeleePadButtonZ]
          defaultFrame:zDefault
            identifier:@"Z"];
    CGFloat startWidth = (pad ? 116.0 : 92.0 * baseScale) * controlScale;
    CGRect startDefault = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.0902222222, 0.1128941441,
                                      startWidth, small) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.8967789165, 0.5780765253,
                                      startWidth, small) :
        CGRectMake(CGRectGetMidX(safe) - startWidth * 0.5,
                   CGRectGetMinY(safe) + margin, startWidth, small);
    [self placeControl:[self buttonWithMask:MeleePadButtonStart]
    ...
    CGRect defaultGroupFrame = phone ?
        MeleePadFrameAtNormalizedCenter(safe, 0.0812777778, 0.4677364865,
                                      3.0 * d, 3.0 * d) : pad ?
        MeleePadFrameAtNormalizedCenter(safe, 0.2686676428, 0.7947259566,
                                      3.0 * d, 3.0 * d) :
...
- (CGFloat)experimentalDPadScale {
    NSNumber *saved = [[NSUserDefaults standardUserDefaults]
        objectForKey:MeleePadExperimentalDPadScaleKey];
...
- (void)placeExperimentalDPadGroupWithDefaultFrame:(CGRect)defaultFrame safeArea:(CGRect)safe {
    ...
    NSString *savedPoint = [[NSUserDefaults standardUserDefaults]
        stringForKey:MeleePadExperimentalDPadOriginKey];
...
- (void)layoutExperimentalDPadButtons {
    ...
    struct {
        ...
        {MeleePadButtonDpadUp, 0.0, -1.0},
        {MeleePadButtonDpadDown, 0.0, 1.0},
        {MeleePadButtonDpadLeft, -1.0, 0.0},
        {MeleePadButtonDpadRight, 1.0, 0.0},
    };
    for (const auto &placement : placements) {
        MeleePadGameButton *button = [self buttonWithMask:placement.mask];
...
- (void)placeControl:(UIView *)control defaultFrame:(CGRect)defaultFrame identifier:(NSString *)identifier {
    NSDictionary *savedScales = [[NSUserDefaults standardUserDefaults]
        dictionaryForKey:@"MeleePadControlSizeScales"];
    CGFloat individualScale = savedScales[identifier] != nil
        ? [[MeleePadSettings sharedSettings] sizeScaleForControl:identifier]
        : MeleePadDefaultSizeScaleForControl(self, identifier);
    ...
    NSDictionary *saved = [[NSUserDefaults standardUserDefaults]
        dictionaryForKey:@"MeleePadControlOrigins"];
...
- (MeleePadGameButton *)buttonWithMask:(uint16_t)mask {
    for (MeleePadGameButton *button in _buttons) {
...
- (void)buildSettingsPanel {
    ...
    _renderScaleControl.selectedSegmentIndex = [MeleePadSettings sharedSettings].renderScale - 1;
    ...
    _opacitySlider.value = [MeleePadSettings sharedSettings].controlOpacity;
    ...
    _sizeSlider.value = [MeleePadSettings sharedSettings].controlSizeScale;
    ...
    _hideControlsSwitch.on = [MeleePadSettings sharedSettings].hideTouchControlsWhenControllerConnected;
    ...
    _modernCStickSwitch.on = [MeleePadSettings sharedSettings].modernCStickHorizontal;
    ...
    _editorBar = [MeleePadPassThroughView new];
...
- (void)renderScaleChanged:(UISegmentedControl *)control {
    [MeleePadSettings sharedSettings].renderScale = control.selectedSegmentIndex + 1;
    [[MeleePadSettings sharedSettings] synchronize];
    [self refreshMenuButton];
}

- (void)opacityChanged:(UISlider *)slider {
    [MeleePadSettings sharedSettings].controlOpacity = slider.value;
    [[MeleePadSettings sharedSettings] synchronize];
    [self setNeedsLayout];
}

- (void)sizeChanged:(UISlider *)slider {
    [MeleePadSettings sharedSettings].controlSizeScale = slider.value;
    [[MeleePadSettings sharedSettings] synchronize];
    [self setNeedsLayout];
}

- (void)hideChanged:(UISwitch *)switcher {
    [MeleePadSettings sharedSettings].hideTouchControlsWhenControllerConnected = switcher.on;
    [[MeleePadSettings sharedSettings] synchronize];
    [self applyControllerVisibility];
}

- (void)modernCStickChanged:(UISwitch *)switcher {
    [MeleePadSettings sharedSettings].modernCStickHorizontal = switcher.on;
    [[MeleePadSettings sharedSettings] synchronize];
}
...
- (void)resetLayout {
    ...
    [defaults removeObjectForKey:MeleePadExperimentalDPadOriginKey];
    [defaults removeObjectForKey:MeleePadExperimentalDPadScaleKey];
    [defaults removeObjectForKey:@"MeleePadControlOrigins"];
    [defaults removeObjectForKey:@"MeleePadControlSizeScales"];
    [defaults removeObjectForKey:@"MeleePadControlSizeScale"];
    [defaults removeObjectForKey:@"MeleePadControlOpacity"];
    [[MeleePadSettings sharedSettings] resetControlSizeScales];
    [[MeleePadSettings sharedSettings] synchronize];
    [self applySettings];
    [self setNeedsLayout];
}
...
- (BOOL)isDPadButton:(UIView *)control {
    if (![control isKindOfClass:MeleePadGameButton.class])
        return NO;
    uint16_t mask = ((MeleePadGameButton *)control).inputMask;
    return mask == MeleePadButtonDpadUp || mask == MeleePadButtonDpadDown ||
           mask == MeleePadButtonDpadLeft || mask == MeleePadButtonDpadRight;
}
...
- (NSString *)identifierForMask:(uint16_t)mask {
    switch (mask) {
    case MeleePadButtonA: return @"A";
    case MeleePadButtonB: return @"B";
    case MeleePadButtonX: return @"X";
    case MeleePadButtonY: return @"Y";
    case MeleePadButtonZ: return @"Z";
    case MeleePadButtonStart: return @"Start";
    case MeleePadButtonL: return @"L";
    case MeleePadButtonR: return @"R";
    case MeleePadButtonDpadUp: return @"D_U";
    case MeleePadButtonDpadDown: return @"D_D";
    case MeleePadButtonDpadLeft: return @"D_L";
    case MeleePadButtonDpadRight: return @"D_R";
    default: return @"";
    }
}
        ...
        if (control == _experimentalDPadGroup) {
            [[NSUserDefaults standardUserDefaults]
                setObject:NSStringFromCGPoint(normalized)
                   forKey:MeleePadExperimentalDPadOriginKey];
        } else {
            NSMutableDictionary *saved = [[[NSUserDefaults standardUserDefaults]
                dictionaryForKey:@"MeleePadControlOrigins"] mutableCopy];
            if (saved == nil)
                saved = [NSMutableDictionary dictionary];
            saved[identifier] = NSStringFromCGPoint(normalized);
            [[NSUserDefaults standardUserDefaults] setObject:saved forKey:@"MeleePadControlOrigins"];
        }
        [[MeleePadSettings sharedSettings] synchronize];
    }
}
...
- (void)selectControlForEditing:(UIView *)control {
    ...
    _selectedSizeSlider.value = control == _experimentalDPadGroup ?
        ...
        [[MeleePadSettings sharedSettings] sizeScaleForControl:control.accessibilityIdentifier];
    ...
    if (_selectedControl == _experimentalDPadGroup) {
        [[NSUserDefaults standardUserDefaults]
            setDouble:std::clamp<double>(slider.value, 0.60, 1.75)
               forKey:MeleePadExperimentalDPadScaleKey];
    } else {
        [[MeleePadSettings sharedSettings] setSizeScale:slider.value forControl:identifier];
    }
    [[MeleePadSettings sharedSettings] synchronize];
    [self setNeedsLayout];
}
...
- (void)applySettings {
    MeleePadSettings *settings = [MeleePadSettings sharedSettings];
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"MeleePadEditingControlLayout"];
    ...
    _editLayoutSwitch.on = NO;
    [[NSUserDefaults standardUserDefaults]
        removeObjectForKey:@"MeleePadExperimentalTouchControls"];
