...
## 2026-09-01 — PERF-263 synchronized pipeline-persistence replay and correction

- Start a fresh process on the unchanged synchronized executable after the
  prior exact match appeared to populate Dolphin's normal `GALE01.uidcache`.
...
- Correct the initial attribution after inspecting the pinned packed format:
  valid files are 8 header bytes plus 579 bytes per entry. The presumed
  151,552-byte cache leaves a 425-byte remainder and would be rejected. The
  non-recurrence may be variance or an external Metal cache; persistence is not
  yet proven.
- Keep cold/front-end, match/results presentation, and malformed rendering as
  independent failures. Do not add another performance toggle or ship a seed.
- Keep synchronized dual-core unmerged. Next reverse a structurally valid UID
  file against an empty isolated user root, then separately attribute residual
